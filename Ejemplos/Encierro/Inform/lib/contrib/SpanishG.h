...
#Iftrue LIBRARY_STAGE == AFTER_VERBLIB;  ! si es el momento correcto de incluirlo

Message "Incluyendo SpanishG [INFSP 6.12]";
...
! importar cual fuera el modo inicial real del juego.
Verb meta 'normal'
  *                                   -> LModeNormal;
VerboIrregular "volver al modo de descripciones inicial" with imperativo 'normal';
...
Verb 'lleva' 'sujeta'
  * multi                             -> Take
  * multiinside 'de' noun             -> Remove;
VerboIrregular "llevar" with imperativo 'lleva' 'sujeta';
...
! actor no lo tiene todavia), pero con mensajes menos naturales que coger
! directo. Ahora el segundo noun=player se resuelve como Take.
Verb 'transfiere' 'cambia'
    * noun 'a//' noun=NotPlayer            -> Transfer
    * noun 'a//' noun=IsPlayer             -> Take;
...
Verb 'palanca'
    * noun 'con' held                    -> Unlock
    * 'aparte'/'abre' noun 'con' held     -> Unlock
    * noun 'aparte'/'abre' 'con' held     -> Unlock;
VerboIrregular "hacer palanca en" with imperativo 'palanca';
Verb 'cierra'
    * noun                               -> Close
...
! [2026-09-16] Progreso real de LIBRARY_STAGE (ver el guard al principio
! del archivo) + los 3 mensajes de error/warning que usa inform6lib en
! grammar.h para el mismo caso: orden incorrecto, doble inclusión, o
! Parser/Verblib ausentes.
Undef LIBRARY_STAGE; Constant LIBRARY_STAGE = AFTER_GRAMMAR;

#Ifnot; ! LIBRARY_STAGE < AFTER_GRAMMAR pero ~= AFTER_VERBLIB
Message "Error: 'Verblib' necesita incluirse correctamente antes de incluir 'SpanishG'. Esto va a causar muchos errores.";
#Endif;

#Ifnot; ! LIBRARY_STAGE >= AFTER_GRAMMAR: ya estaba incluido
Message "Warning: 'SpanishG' incluido dos veces; se ignora la segunda inclusión. (Ignorar si es a propósito.)";
#Endif;

#Ifnot; ! LIBRARY_STAGE no está definido
Message "Error: 'Parser' y 'Verblib' necesitan incluirse correctamente antes de incluir 'SpanishG'. Esto va a causar muchos errores.";
#Endif;
