...
!
!  INFSP es traduccion al ingles de informATE (by Sarganar)
!  contiene codigo aportado de diveras fuentes
!  InformATE es (c) de Zak
!
!  github.com/sarganar/infsp6
! ----------------------------------------------------------------------------

System_file;

! [2026-09-16] Guard de inclusión -- mismo patrón que inform6lib/english.h
! (#Ifndef LIBRARY_ENGLISH ... Constant LIBRARY_ENGLISH; #Endif;)
#Ifndef LIBRARY_SPANISH;

Message "^Incluyendo Spanish: Mensajes y rutinas de idioma [INFSP 6.12]";
...
Constant LanguageVersion = "Librería Española INFSP 6.12, basada en InformATE!";
...
!---------------------------------------------------------------------------
! [item #9] Mensajes que dependen de la persona narrativa (narrative_voice/
! narrative_number). Global -- no Constant -- porque AsignarPersona (mas
! abajo, junto a la seccion de Mensajes) los reasigna en tiempo de
! ejecucion. Declarados aca (antes de LanguageVerb, que ya usa
! PARTICULA_TE) porque Inform 6 exige que un Global se declare antes de
! su primer uso. Valores iniciales = los de siempre (2a persona singular),
! asi que un juego que nunca llama a AsignarPersona anda igual que antes.
!---------------------------------------------------------------------------
...
! [2026-09-22] Para no repetir el aviso de AsignarPersona en cada turno.
Global AvisoPersonaSinShortName = false;
...
! [item #9] Complementa narrative_voice/narrative_tense de inform6lib
! (Parser.h, SelfClass): el español conjuga distinto en plural y el inglés
! de inform6lib no lo necesita, así que no viene declarada ahí.

Constant SINGULAR_NUMBER = 1;
Constant PLURAL_NUMBER   = 2;
Global narrative_number = SINGULAR_NUMBER;
...
! bytes de ISO-8859-1 -- son un orden propio del estandar. Se verifico que
! esa tabla incluye los acentuados que necesita el espanol (a-e-i-o-u con
! tilde, n con tilde, u con dieresis) y sus mayusculas, asi que no hace falta
! adaptar los rangos, solo portar la rutina tal cual.
! Necesaria para PasarAMinuscula (mas abajo), que hasta ahora llamaba a
! LowerCase sin que existiera -- rompia cualquier compilacion a Glulx.
#Ifdef TARGET_ZCODE;
...
[ LanguageVerb i aux;
    ...
    if (ImprimirIrregular(i)==0){
      ...
      ! en este mismo archivo y se usa igual (obj.door_dir) en PrintCommand
      ! mas abajo, asi que solo hacia falta descomentarla.
...
[ LanguageVerbLikesAdverb w;
    ...
    if (w == 'vete' or 've' or 'camina' or
        'anda' or 'corre' or 'vuelve' or 'ir' or
        'empuja' or 'mueve' or 'desplaza' or 'menea')
        rtrue;
    rfalse;
];
...
! Definite article (default player object; redefine for feminine protagonist)
Constant THE__TX = "el";

! Past tense forms (grammatically correct but dormant — INFSP has no PAST_TENSE)
Constant WAS__TX   = " veías";
Constant WERE__TX  = " veíais";
Constant WAS2__TX  = "veías ";
Constant WERE2__TX = "veíais ";

! Punctuation
Constant COLON__TX = ":";

! EnterSub verb tokens
Constant STAND__TX = 'levantate';
Constant SIT__TX   = 'sienta';
Constant LIE__TX   = 'echate';

! Version / library identification (English — matches Inform tooling conventions)
...
!-------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
! [2026-09-22] Infraestructura que NO son mensajes, y por eso vive FUERA del
! bloque SIN_MENSAJES de mas abajo.
!
! Estaba adentro, y eso rompia la compilacion de cualquier juego que definiera
! SIN_MENSAJES: inform6lib referencia estos simbolos SIEMPRE, se incluyan o no
! los mensajes de la libreria.
!
!   YOURSELF__TX    Parser.h:823  (short_name de selfobj) y Parser.h:7128
!   MYSELF__TX      Parser.h:7127
!   MYFORMER__TX    Parser.h:5855, y INFSPR.h
!   ThatOrThose()   Parser.h:7133
!   Tense()         Verblib.h:336-337 y 686-687 (list writer)
!   AsignarPersona() API publica del juego, y la llama ChangePlayer en INFSPR.h
!
! Los ~35 helpers de conjugacion (as_, es_, tu_, ...) SI quedan adentro: solo
! los usa LanguageLM, que es justamente lo que SIN_MENSAJES reemplaza.
!-------------------------------------------------------------------------------
...
! =============================================================================
! [item #9 de la sync] Infraestructura de narrative_voice/narrative_tense
! =============================================================================
! inform6lib (Parser.h) ya declara 'narrative_voice' (1/2/3) y
! 'narrative_tense' (PRESENT_TENSE/PAST_TENSE) como properties de
! SelfClass/selfobj, con default 2 (segunda persona) / PRESENT_TENSE -- no
! hace falta redeclararlas. 'narrative_number' (propia, ver mas arriba)
! completa lo que a inform6lib no le hace falta en inglés.

! -----------------------------------------------------------------------
! AsignarPersona: adapta YOU__TX/player.short_name/CANTGO__TX/IS__TX/ARE__TX/
! IS2__TX/ARE2__TX/PARTICULA_TE/FORMER__TX al player.narrative_voice /
! narrative_number (Global) / genero (gender, G_FEMENINO) actuales.
!
! Basada en el trabajo de Ricpelo (infsp6_fix_byRicpelo/Spanish.h,
! AsignarPersona), adaptada para leer narrative_voice (property estandar de
! inform6lib) en vez de un 'persona' propio -- asi los 2 puntos de Parser.h
! que ya miran narrative_voice directamente (SelfClass, el "mi antiguo yo"
! al cambiar de jugador) siguen funcionando sin parches extra. Se
! corrigieron 2 bugs de la referencia original al
! portarla: (a) la rama de 1a persona SINGULAR asignaba YOU__TX/
! YOURSELF__TX en plural ("Nosotros"/"nosotros mismos") por un copy-paste
! de la rama plural -- deberia ser "Yo"/"yo mismo"; (b) la rama de 3a
! persona SINGULAR usaba 'le' (clitico de objeto indirecto) para
! PARTICULA_TE, pero el unico uso real de PARTICULA_TE en la libreria
! (LanguageVerb, eco reflexivo del verbo) es el clitico REFLEXIVO, que en
! espanol es 'se' tanto en singular como en plural -- la rama plural ya
! usaba 'se' correctamente.
!
! No se llama automaticamente salvo una excepcion: los valores
! iniciales de los Global de arriba ya son 2a persona singular, asi que
! un juego que nunca toca narrative_voice/narrative_number/
! narrative_tense sigue andando igual que siempre. Para narrar en 1a/3a
! persona o en plural:
!   1. asignar player.narrative_voice / narrative_number (Global)
!      (y opcionalmente player.narrative_tense = PAST_TENSE)
!   2. llamar a AsignarPersona()
! [2026-09-17 backlog #4] la excepcion es ChangePlayer (INFSPR.h,
! Replace-ada): la llama sola despues de cambiar player, mismo patch
! que ya tenia el fork de Ricpelo. Sigue siendo opt-in en todo lo
! demas -- si un juego cambia de PC y quiere una voz/numero distintos
! para el nuevo cuerpo, igual tiene que asignarlos ANTES de invocar
! ChangePlayer (o volver a llamar AsignarPersona() despues a mano si
! los cambia recien ahi).
! -----------------------------------------------------------------------
[ AsignarPersona  femenino plural;
    if (~~(player provides narrative_voice)) return;
    ! [2026-09-22] Todas las ramas de abajo escriben player.short_name, asi
    ! que no alcanza con haber chequeado narrative_voice: un objeto-jugador
    ! que declare la voz pero no short_name pasaba el guard y se caia aca
    ! con un "Fatal error: No such property" que no nombra ni esta rutina ni
    ! la propiedad -- compila limpio y muere antes de la primera linea.
    ! Pasa con un molde de jugador propio declarado sin nombre entre comillas
    ! (un "Objeto X Sala" sin el "nombre" no recibe short_name implicito).
    ! Avisamos y seguimos en vez de abortar. Los helpers de conjugacion
    ! leen player.narrative_voice por su cuenta, asi que el texto de los
    ! mensajes sale bien igual: lo unico que queda sin fijar es como se
    ! nombra el jugador a si mismo (~yo mismo~ / ~ti mismo~).
    if (~~(player provides short_name)) {
        if (AvisoPersonaSinShortName == false) {
            AvisoPersonaSinShortName = true;
            print "^[INFSP] AsignarPersona: el objeto-jugador actual declara
                narrative_voice pero no short_name, asi que no puedo fijar
                como se nombra a si mismo (~yo mismo~, ~ti mismo~). Haz que
                su clase derive de SelfClass (Class MiJugador class
                SelfClass ...) o dale un short_name propio.^";
        }
        return;
    }
    femenino = (player has female ||
        (player provides gender && player.gender & G_FEMENINO));
    plural = (narrative_number == PLURAL_NUMBER);
    ...
    switch (player.narrative_voice) {
      3: ! Tercera persona
        if (plural) {
            ...
            IS__TX = " ven"; ARE__TX = " ven";
            IS2__TX = "ven "; ARE2__TX = "ven ";
            PARTICULA_TE = "se";
            if (femenino) { player.short_name = "ellas mismas"; YOU__TX = "Ellas"; }
            else          { player.short_name = "ellos mismos"; YOU__TX = "Ellos"; }
        } else {
            FORMER__TX = "su antiguo ~yo~";
            CANTGO__TX = "No puede ir por ahí.";
            IS__TX = " ve"; ARE__TX = " ve";
            IS2__TX = "ve "; ARE2__TX = "ve ";
            PARTICULA_TE = "se"; ! reflexivo: 'se', no 'le' (indirecto)
            if (femenino) { player.short_name = "ella misma"; YOU__TX = "Ella"; }
            else          { player.short_name = "él mismo";   YOU__TX = "Él"; }
        }
      1: ! Primera persona
        if (plural) {
            FORMER__TX = "nuestro antiguo ~yo~";
            CANTGO__TX = "No podemos ir por ahí.";
            IS__TX = " vemos"; ARE__TX = " vemos";
            IS2__TX = "vemos "; ARE2__TX = "vemos ";
            PARTICULA_TE = "nos";
            if (femenino) { player.short_name = "nosotras mismas"; YOU__TX = "Nosotras"; }
            else          { player.short_name = "nosotros mismos"; YOU__TX = "Nosotros"; }
        } else {
            FORMER__TX = "mi antiguo ~yo~";
            CANTGO__TX = "No puedo ir por ahí.";
            IS__TX = " veo"; ARE__TX = " veo";
            IS2__TX = "veo "; ARE2__TX = "veo ";
            PARTICULA_TE = "me";
            if (femenino) player.short_name = "yo misma"; else player.short_name = "yo mismo";
            YOU__TX = "Yo";
        }
      default: ! Segunda persona (valor 2, o cualquier otro no reconocido)
        if (plural) {
            FORMER__TX = "vuestro antiguo ~yo~";
            CANTGO__TX = "No podéis ir por ahí.";
            IS__TX = " veis"; ARE__TX = " veis";
            IS2__TX = "veis "; ARE2__TX = "veis ";
            PARTICULA_TE = "os";
            if (femenino) { player.short_name = "vosotras mismas"; YOU__TX = "Vosotras"; }
            else          { player.short_name = "vosotros mismos"; YOU__TX = "Vosotros"; }
        } else {
            FORMER__TX = "tu antiguo ~yo~";
            CANTGO__TX = "No puedes ir por ahí.";
            IS__TX = " ves"; ARE__TX = " ves";
            IS2__TX = "ves "; ARE2__TX = "ves ";
            PARTICULA_TE = "te";
            if (femenino) player.short_name = "ti misma"; else player.short_name = "ti mismo";
            YOU__TX = "Tú";
        }
    }
];
...
! -----------------------------------------------------------------------
! Tense: igual que inform6lib/english.h -- imprime la forma en presente o
! en pasado de un verbo segun player.narrative_tense. El espanol no
! necesita nada mas elaborado aca (la complejidad esta en la PERSONA, no
! en el tiempo): PRESENT_TENSE/PAST_TENSE ya vienen de Parser.h.
! -----------------------------------------------------------------------
[ Tense present past;
...
! -----------------------------------------------------------------------
! ThatOrThose: existe en inform6lib/english.h (imprime "that"/"those"/
! "him"/"her"/"me" al referirse a un objeto). No la teniamos -- sin ella
! Parser.h/Verblib.h no compilan (ver Indefart en Parser.h, que la usa, y
! que Inform 6 compila-y-descarta igual por el "Replace"). Portada con la
! misma estructura, usando THAT__TX/THOSET__TX que ya existian en esta
! libreria. AVISO: hoy ningun mensaje en espanol la llama todavia (eso es
! el ítem #8, reescribir LanguageLM) -- las formas "mí"/"ti" para el caso
! narrative_voice==1/2 son una eleccion razonable pero provisional, sin
! un call site real que las ejercite para confirmar el caso gramatical
! correcto (objeto directo vs. termino de preposicion). Revisar cuando
! #8 escriba los primeros mensajes que la usen de verdad.
! -----------------------------------------------------------------------
[ ThatOrThose obj;
    if (obj == player) {
        if (player provides narrative_voice) {
            if (player.narrative_voice == 1) { print "mí"; return; }
            if (player.narrative_voice == 3) { print (PSN__) player; return; }
        }
        print "ti"; return;
    }
    if (obj has pluralname) { print (string) THOSET__TX; return; }
    if (obj has female)     { print "ella"; return; }
    if (obj has male or animate)
        if (obj hasnt neuter) { print "él"; return; }
...
! =============================================================================
! [item #8 de la sync] Helpers de conjugacion para narrative_voice/tense
! =============================================================================
! Cada rutina imprime la terminacion (o, en las de raiz irregular, la palabra
! completa) que corresponde a player.narrative_voice/narrative_number, y
! despues imprime x (el resto del mensaje) -- se usan como "print rule"
! embebida en LanguageLM: '"Cort", (as_) " la cuerda."' imprime "Cortas la
! cuerda." en 2a persona singular, "Corto la cuerda." en 1a, etc.
!
! Portadas de infsp6_fix_byRicpelo/Spanish.h (mismos 34 nombres y mismas
! formas verbales -- ahi esta el trabajo lingueistico real), adaptadas para
! leer player.narrative_voice (1/2/3, property estandar de inform6lib) +
! narrative_number (Global propio, ver AsignarPersona arriba) en vez del
! bitmask 'persona' propio de esa referencia. Sin uso todavia (eso es
! LanguageLM, mas abajo) -- declaradas antes para poder llamarlas desde ahi.
!
! Tabla de referencia (terminaciones puras; no aplica a las de raiz
! irregular como tienes_/_Cierras_/puedes_/pueda_/sientes_/encuentro_/
! eres_/hueles_/sueltes_, que cambian toda la palabra):
!
!            ti_      se_  te_  tu_      as_   es_   _es_  oy_  as__ a_   eo_  e_    os_ z_  ias_
!   1sg      mi       me   me   mi       o     o     o     oy   e    a    eo   e     o   z   ia
!   2sg      ti       te   te   tu       as    es    es    as   as   as   es   as    o   z   ias
!   3sg      si       se   le   su       a     e     e     a    a    a    e    a     o   z   ia
!   1pl      nosotr@  nos  nos  nuestr@  amos  emos  imos  amos emos amos emos emos  os  ces iamos
!   2pl      vosotr@  os   os   vuestr@  ais   eis   is    ais  eis  ais  eis  abeis os  ces iais
!   3pl      ell@     se   les  su       an    en    en    an   an   an   en   an    os  ces ian
! =============================================================================


[ ActorNarrativeVoice;
    if (actor ~= player) return 3;
    if (player provides narrative_voice) return player.narrative_voice;
    return 0; ! sin narrative_voice: cada helper tiene su propio fallback
];

[ ActorNarrativeNumber;
    if (actor ~= player) {
        if (actor has pluralname) return PLURAL_NUMBER;
        return SINGULAR_NUMBER;
    }
    return narrative_number;
];

[ ti_ x femenino voice;
    voice = ActorNarrativeVoice();
    if (voice) {
        femenino = (player has female ||
            (player provides gender && player.gender & G_FEMENINO));
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "nosotr";
                 if (femenino) print "as"; else print "os";
              3: print "ell";
                 if (femenino) print "as"; else print "os";
              default: print "vosotr"; ! Segunda persona
                 if (femenino) print "as"; else print "os";
            }
        } else {
            switch (voice) {
              1: print "mí";
              3: print "sí";
              default: print "ti"; ! Segunda persona
            }
        }
    } else print "ti";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "nos";
              3: print "se";
              default: print "os"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "me";
              3: print "se";
              default: print "te"; ! Segunda persona
            }
        }
    } else print "te";
    print (string) x;
...
! _Refl_: version capitalizada del reflexivo se_ (Me/Te/Se/Nos/Os/Se), para
! arrancar oracion con un verbo reflexivo ("Se pone el abrigo.", "Te
! quitas la camisa."). Distinta de _Te_ (Me/Te/Le/Nos/Os/Les, objeto
! indirecto -- "Te falta destreza."). NO puede llamarse "_Se_": Inform 6
! es case-insensitive para identificadores, y colisionaria con la rutina
! "_se_" ya existente (verbo "saber": se/sabes/sabe...).
[ _Refl_ x voice;
    voice = ActorNarrativeVoice();
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
...
! [2026-09-17 backlog #2] tu_/_Tu_ son para sustantivos poseidos
! MASCULINOS (unico caso real hoy: "soplido" en Blow). En plural
! ("nuestro"/"vuestro") el genero debe concordar con el SUSTANTIVO
! poseido, no con el jugador -- versiones anteriores leian
! player.gender aca, lo cual daba "nuestro"/"vuestro" incluso para un
! sustantivo femenino con jugador varon. Como se llaman via la sintaxis
! de lista de impresion "(rutina) valor" (un solo argumento = el texto
! que sigue), no hay forma de pasarles el genero del sustantivo como
! segundo parametro real. Para un sustantivo FEMENINO, usar
! TuFemenino_ (ver mas abajo) en vez de estas -- mismo idiom que
! Ricpelo ya habia escrito a mano en Pray ("oraciones").
[ tu_ x voice;
    voice = ActorNarrativeVoice();
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "nuestro";
              3: print "su";
              default: print "vuestro"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "mi";
              3: print "su";
              default: print "tu"; ! Segunda persona
            }
        }
    } else print "tu";
    print (string) x;
...
! [2026-09-17 backlog #2] variante de tu_ para sustantivos poseidos
! FEMENINOS en plural (unico caso real hoy: "puntuacion" en Score).
! Mismo idiom que Pray ("oraciones"), ahora factorizado en rutina en
! vez de repetido a mano porque Score lo necesita dos veces.
[ TuFemenino_ x voice;
    voice = ActorNarrativeVoice();
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "nuestra";
              3: print "su";
              default: print "vuestra"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "mi";
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "amos";
              3: print "an";
              default: print "áis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "o";
              3: print "a";
              default: print "as"; ! Segunda persona
            }
        }
    } else print "as";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "emos";
              3: print "en";
              default: print "éis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "o";
              3: print "e";
              default: print "es"; ! Segunda persona
            }
        }
    } else print "es";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "imos";
              3: print "en";
              default: print "ís"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "o";
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            ...
            switch (voice) {
              1: print "oy";
              3: print "a";
              default: print "ás"; ! Segunda persona
            }
        }
    } else print "ás";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "emos";
              3: print "an";
              default: print "abéis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "e";
...
! os_/z_: solo dependen del numero, no de la voz (participios/adjetivos
! invariables entre personas, solo cambian singular/plural).

[ os_ x;
    if (ActorNarrativeVoice() && ActorNarrativeNumber() == PLURAL_NUMBER)
        print "os";
    else
        print "o";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "íamos";
              3: print "ían";
              default: print "íais"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "ía";
              3: print "ía";
              default: print "ías"; ! Segunda persona
            }
        }
    } else print "ías";
    print (string) x;
...
! ues_: "o" solo en 1a persona singular (huyo, construyo...); el resto usa
! "u" + la terminacion -ir de (_es_) (huyes/huye/huimos/huis/huyen).

[ ues_ x;
    if (ActorNarrativeVoice() == 1 && ActorNarrativeNumber() ~= PLURAL_NUMBER)
        print "o", (string) x;
    else
        print "u", (_es_) x;
];

! sales_/_Sales_/pones_: raiz con 'g' epentetica solo en 1a persona singular
! (salgo/pongo), el resto de personas usa la raiz plana + (_es_)/(es_).
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "tene";
              3: print "tien";
              default: print "ten"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "teng";
              3: print "tien";
              default: print "tien"; ! Segunda persona
            }
        }
    } else print "tien";
    print (es_) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "soltemos";
              3: print "suelten";
              default: print "soltéis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "suelte";
              3: print "suelte";
              default: print "sueltes"; ! Segunda persona
            }
        }
    } else print "sueltes";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "Cerramos";
              3: print "Cierran";
              default: print "Cerráis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "Cierro";
              3: print "Cierra";
              default: print "Cierras"; ! Segunda persona
            }
        }
    } else print "Cierras";
    print (string) x;
...
! [2026-09-16] Versión minúscula de _Cierras_ (misma diptongación e/ie,
! "cerrar" no sigue el patrón regular de as_/es_), para usarla a mitad de
! frase -- p.ej. "(primero cierras la puerta)". Agregada al portar
! Close caso 4.
[ cierras_ x voice;
    voice = ActorNarrativeVoice();
    if (voice) {
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "podemos";
              3: print "pueden";
              default: print "podéis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "puedo";
              3: print "puede";
              default: print "puedes"; ! Segunda persona
            }
        }
    } else print "puedes";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "podamos";
              3: print "puedan";
              default: print "podáis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "pueda";
              3: print "pueda";
              default: print "puedas"; ! Segunda persona
            }
        }
    } else print "puedas";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "sentimos";
              3: print "sienten";
              default: print "sentís"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "siento";
              3: print "siente";
              default: print "sientes"; ! Segunda persona
            }
        }
    } else print "sientes";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "encontramos";
              3: print "encuentran";
              default: print "encontráis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "encuentro";
              3: print "encuentra";
              default: print "encuentras"; ! Segunda persona
            }
        }
...
! _se_: "sé" (verbo saber, 1a persona singular) es irregular; el resto usa
! la raiz "sab" + (es_) (sabes/sabe/sabemos/sabeis/saben).
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "somos";
              3: print "son";
              default: print "sois"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "soy";
              3: print "es";
              default: print "eres"; ! Segunda persona
            }
        }
    } else print "eres";
    print (string) x;
    ...
    if (voice) {
        if (ActorNarrativeNumber() == PLURAL_NUMBER) {
            switch (voice) {
              1: print "olemos";
              3: print "huelen";
              default: print "oléis"; ! Segunda persona
            }
        } else {
            switch (voice) {
              1: print "huelo";
              3: print "huele";
              default: print "hueles"; ! Segunda persona
            }
        }
    } else print "hueles";
    print (string) x;
...
! [2026-09-16 backlog #6] Equivalente a DecideAgainst() de inform6lib
! (english.h:755): mensaje generico de "mejor no lo intento" que varias
! acciones usan para su caso n=2 (Jump{In,On,Over}, Tie -- ver
! LanguageLM). El ingles lo nombra siempre (CSubjectVerb(actor,...)); el
! español hace lo mismo acá.
[ DecidirEnContra;
    if (actor == player) "Decid", (_es_) " que no es tan buena idea.";
    else "", (The) actor, " decid", (_es_) " que no es tan buena idea.";
];

[ LanguageLM n x1 x2;
 ! [2026-09-16] agregado el parámetro x2: L__M/L___M (inform6lib,
 ! verblib.h) siempre llaman LanguageLM(n, x1, x2) con 3 argumentos;
 ! Inform 6 descarta en silencio los de más si la rutina declara menos,
 ! así que nunca hizo falta hasta que Miscellany 58 necesitó el segundo
 ! objeto (el contenedor/soporte del que se saca x1).
 ...
 ! Ask:      see Answer
  ...
  Close:    switch(n)    {
     ...
     3: ! [2026-09-16 backlog #6] nombra al actor si no es el jugador,
        ! igual que CSubjectVerb(actor,...) en inform6lib/english.h:829.
        if (actor == player) "", (_Cierras_) " ", (the) x1, ".";
        else "", (The) actor, " ", (cierras_) " ", (the) x1, ".";
     4: ! [2026-09-16] Faltaba -- lo llama ImplicitClose (verblib.h,
        ! heredado) para avisar que cerró algo de paso antes de otra
        ! acción (mismo patrón que Miscellany 58/Disrobe 3).
        "(primero ", (cierras_) " ", (the) x1, ")";
    }
  ...
  Consult: ! [2026-09-16 backlog #6] nombra al actor si no es el jugador,
    ! igual que CSubjectVerb(actor,...) en inform6lib/english.h:856.
    if (actor == player) "No descubr", (_es_) " nada interesante en ", (the) x1,
              " sobre ese tema.";
    else "", (The) actor, " no descubr", (_es_) " nada interesante en ",
              (the) x1, " sobre ese tema.";
  Cut:      "Cortándo", (lo) x1, " no lograr", (as__) " gran cosa.";
  Dig:      "Excavar no servirá de nada aquí.";
  Disrobe:    switch(n)    {
     ! [2026-09-16 backlog #6] casos 1/2/4 nombran al actor si no es el
     ! jugador, igual que CSubjectIsnt/CSubjectVerb(actor,...) en
     ! inform6lib/english.h:870-875. Caso 3 (aviso implícito) no nombra
     ! en inglés tampoco -- se deja como estaba.
     1: if (actor == player) "No llev", (as_) " puesto eso.";
        else "", (The) actor, " no llev", (as_) " puesto eso.";
     2: if (actor == player) "", (_Refl_) " quit", (as_) " ", (the) x1, ".";
        else "", (The) actor, " ", (se_) " quit", (as_) " ", (the) x1, ".";
     3: ! [2026-09-16] Faltaba este caso -- es el que usa ImplicitDisrobe
        ! (verblib.h) para avisar que primero hay que quitarse la prenda
        ! antes de otra acción (p.ej. "dejar camisa" estando puesta). Texto
        ! rescatado de donde vivía por error (Drop caso 3, ver esa nota).
        "(primero ", (se_) " quit", (as_) " ", (the) x1, ")";
     4: ! no_implicit_actions activo (caso raro): hay que quitarse la
        ! prenda a mano antes.
        if (actor == player) "", (_Tienes_) " que quitar", (se_) " primero ",
            (the) x1, ".";
        else "", (The) actor, " ", (tienes_) " que quitar", (se_) " primero ",
            (the) x1, ".";
    }
  Drink:       "Eso no parece potable.";
  Drop:
    ! [2026-09-16] Renumerado para que coincida con los call sites
    ! reales de inform6lib (DropSub/PutOnSub/InsertSub en verblib.h,
    ! heredado, no reemplazado por INFSP): la numeración vieja de esta
    ! librería (1/2 = errores propios, 3 = aviso de desnudarse, 4 =
    ! éxito) no correspondía al esquema que usa hoy inform6lib
    ! (1 = ya está ahí, 3 = éxito, 4 = habría que sacarlo primero) --
    ! el síntoma real era que "dejar" un objeto normal (no puesto)
    ! imprimía el viejo mensaje 3 ("primero te quitas X") en vez de
    ! "Dejado.", porque DropSub llama L__M(##Drop, 3, noun) esperando
    ! el mensaje de éxito. El aviso de desnudarse en sí sigue andando
    ! bien porque pasa por ##Disrobe, no por acá.
    !   1: X ya está ahí -- reusado también por PutOn/Insert cuando el
    !      objeto ya está sobre/dentro del objetivo.
    !   2: sin usar por ningún call site actual de inform6lib (se deja
    !      por compatibilidad, igual que en la versión inglesa).
    !   3: Éxito -- "Dejado."/"Dejada."
    !   4: no_implicit_actions activo (caso raro): hay que sacar x1 de
    !      x2 a mano antes de poder dejarlo.
    switch(n)
    {
     1: print (The) x1, " ya ", (esta) x1, " ahí.";
     ! [2026-09-16 backlog #6] casos 2/4 nombran al actor si no es el
     ! jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:882/886. Caso 3 ("Dropped.") no nombra en
     ! inglés tampoco -- se deja como estaba.
     2: if (actor == player) "No ", (lo) x1, " ", (tienes_) ".";
        else "", (The) actor, " no ", (lo) x1, " ", (tienes_) ".";
     3: "Dejad", (o) x1, ".";
     4: if (actor == player) {
            print (_Tienes_) " que coger primero ";
        } else {
            print (The) actor, " ", (tienes_) " que coger primero ";
        }
        if (x1 has animate) print (al) x1; else print (the) x1;
        if (x2 has supporter) print " de encima de "; else print " de dentro de ";
        print (the) x2, " antes de dejar", (lo) x1, ".";
        rtrue;
    }
  Eat:
    ...
    switch(n)
    {
     ...
     ! [2026-09-16 backlog #6] nombra al actor si no es el jugador, igual
     ! que CSubjectVerb(actor,...) en inform6lib/english.h:896 -- ese
     ! mismo call site tampoco agrega el aparte "Not bad."/"No está mal."
     ! cuando actor~=player, así que se replica esa condición acá.
     2: if (actor == player) "", (_Refl_) " com", (es_) " ", (the) x1,
            ". No está mal.";
        else "", (The) actor, " ", (se_) " com", (es_) " ", (the) x1, ".";
    }
  EmptyT:
    ...
    switch(n)
    {
      ...
      4:
        "No ", (puedes_) " vaciar un objeto sobre sí mismo."; ! [INFSP] antes usaba x1
    }
  Enter: ! [infsp] GoIn (Meterse) se cambio por Enter.
    ...
    ! [2026-09-16 backlog #6] casos 1/3/4/5 nombran al actor si no es el
    ! jugador, igual que CSubjectIs/CSubjectCant/CSubjectVerb(actor,...)
    ! en inform6lib/english.h:908/919/921/923. Caso 2 (subject=x1, actor
    ! solo mencionado de rebote) y casos 6/7 (avisos implícitos,
    ! impersonales también en inglés) no nombran -- se dejan como estaban.
    switch(n) {
     1: if (actor == player) print "Pero si ya est", (oy_) " ";
        else print "Pero si ", (the) actor, " ya est", (oy_) " ";
    if (x1 has supporter) print "sobre "; else print "en ";
    print_ret (the) x1, ".";
     2: print "No es algo donde ", (pueda_) " "; LanguageVerb(verb_word); ".";
     3: if (actor == player) "No ", (puedes_) " entrar en ", (the) x1,
            " porque está", (n) x1, " cerrad", (o) x1, ".";
        else "", (The) actor, " no ", (puedes_) " entrar en ", (the) x1,
            " porque está", (n) x1, " cerrad", (o) x1, ".";
     4: if (actor == player) print "No ", (puedes_) " ";
        else print (The) actor, " no ", (puedes_) " ";
        LanguageVerb(verb_word);
        print_ret " ahí mientras no lo ", (sueltes_) ".";
     5:
    if (actor == player) {
        if (x1 has supporter) print "Sub", (_es_) " ", (al) x1;
    ...
    print_ret ".";
     6:
    if (x1 has supporter) print "(", (se_) " baj", (as_) " "; else print "(", (sales_) " ";
    print (del) x1; ")";
     7: if (x1 has supporter) "(", (se_) " sub", (_es_) " ", (al) x1, ")^";
    if (x1 has container) "(", (se_) " met", (es_) " en ", (the) x1, ")^";
    "(entr", (as_) " en ", (the) x1, ")^";
    }
  ...
  Examine:
    ...
    switch(n)
    {
     ...
     ! [2026-09-16 backlog #6] nombra al actor si no es el jugador, igual
     ! que CSubjectVerb(actor,...) en inform6lib/english.h:932.
     2: if (actor == player) "No observ", (as_) " nada especial en ",
            (the) x1, ".";
        else "", (The) actor, " no observ", (as_) " nada especial en ",
            (the) x1, ".";
  ...
  Exit:
    ...
    ! [2026-09-16 backlog #6] casos 1/2/3/4/6 nombran al actor si no es
    ! el jugador, igual que CSubjectIsnt/CSubjectCant/CSubjectVerb(actor,
    ! ...) en inform6lib/english.h:940-953. Caso 5 (aviso implícito) no
    ! nombra en inglés tampoco -- se deja como estaba.
    switch(n)
    {
     1: if (actor == player) print "No est", (oy_) " en ningún sitio del que deb", (a_) " ";
        else print (The) actor, " no est", (oy_) " en ningún sitio del que deb", (a_) " ";
    LanguageVerb(verb_word); ".";
     2: if (actor == player) "No ", (puedes_) " salir ", (del) x1,
            " porque está cerrad", (o)x1, ".";
        else "", (The) actor, " no ", (puedes_) " salir ", (del) x1,
            " porque está cerrad", (o)x1, ".";
     3: if (actor == player) {
            if (x1 has supporter) print "Baj", (as_) " ";
            else print (_Sales_) " ";
        } else {
            print (The) actor, " ";
            if (x1 has supporter) print "baj", (as_) " ";
    ...
    print_ret (del) x1, ".";
     4:  if (actor == player) print "Pero es que no est", (oy_) " ";
         else print "Pero es que ", (the) actor, " no est", (oy_) " ";
            if (x1 has supporter) print "encima de  "; else print "dentro de ";
            print_ret (the) x1, ".";
     5: ! [2026-09-16] Faltaba -- lo llama ImplicitExit (verblib.h,
        ! heredado), mismo patrón de anuncio que Close/Open/Miscellany 58.
        print "(primero ";
        if (x1 has supporter) print "baj", (as_) " "; else print (sales_) " ";
        print (del) x1, ")";
     6: ! [2026-09-16] Faltaba -- se usa cuando el jugador tenía la
        ! propiedad "posture" activa (sentado/tumbado) y se levanta al
        ! salir.
        if (actor == player) "", (se_) " levant", (as_) ".";
        else "", (The) actor, " ", (se_) " levant", (as_) ".";
    }
  ...
  GetOff:
    ...
    ! [2026-09-16 backlog #6, tanda 2] nombra al actor si no es el
    ! jugador, igual que CSubjectIsnt(actor,...) en
    ! inform6lib/english.h:970-971.
    if (actor == player) "Pero si no est", (oy_) " en ", (the) x1, ".";
    else "Pero si ", (the) actor, " no est", (oy_) " en ", (the) x1, ".";
  Give:
    ...
    ! [2026-09-16 backlog #6, tanda 2] casos 1/2 nombran al actor si no
    ! es el jugador, igual que CSubjectIsnt/CSubjectVerb(actor,...) en
    ! inform6lib/english.h:973-977. Caso 3 nombra a x1 (el receptor), no
    ! al actor -- ya lo hacía bien. Caso 4 (agregado 2026-09-16, PNJ te
    ! da algo) ya nombraba al actor -- sin cambios.
    switch(n)
    {
     1: if (actor == player) "No ", (tienes_) " ", (the) x1, ".";
        else "", (The) actor, " no ", (tienes_) " ", (the) x1, ".";
     2: if (actor == player) "Manose", (as_) " ", (the) x1,
            " un ratito, pero no consigu", (_es_) " gran cosa.";
        else "", (The) actor, " manose", (as_) " ", (the) x1,
            " un ratito, pero no consigu", (_es_) " gran cosa.";
     3: print_ret (The) x1, " no parece", (n) x1, " interesad", (o)x1, ".";
     4: ! [2026-09-16] Faltaba -- se usa cuando el receptor ERES VOS (un
        ! PNJ te da algo, p.ej. "pepe, dame la llave").
        print_ret (The) actor, " te entrega ", (the) x1, ".";
    }
  Go:
    ...
    ! [2026-09-16 backlog #6, tanda 2] casos 1-6 nombran al actor si no
    ! es el jugador, igual que CSubjectWill/CSubjectCant/CSubjectIs
    ! (actor,...) en inform6lib/english.h:983-990. Caso 7 ya nombraba
    ! (agregado 2026-09-16) -- sin cambios.
    switch(n)
    {
     1: if (actor == player) print (_Tienes_) " que ";
        else print (The) actor, " ", (tienes_) " que ";
         if (x1 has supporter) print "bajar", (se_) " "; else print "salir ";
         print_ret (del) x1, " antes.";
     2: if (actor == player) "No ", (puedes_) " ir por ahí.";
        else "", (The) actor, " no ", (puedes_) " ir por ahí.";
     3: if (actor == player) "", (_Eres_) " incapaz de trepar por ", (the) x1, ".";
        else "", (The) actor, " ", (eres_) " incapaz de trepar por ", (the) x1, ".";
     4: if (actor == player) "", (_Eres_) " incapaz de bajar por ", (the) x1, ".";
        else "", (The) actor, " ", (eres_) " incapaz de bajar por ", (the) x1, ".";
     5: if (actor == player) "No ", (puedes_) " pasar a través ", (del) x1, ".";
        else "", (The) actor, " no ", (puedes_) " pasar a través ", (del) x1, ".";
     6: if (actor == player) "No ", (puedes_) " ir porque ", (the) x1,
            " no lleva", (n) x1, " a ningún sitio.";
        else "", (The) actor, " no ", (puedes_) " ir porque ", (the) x1,
            " no lleva", (n) x1, " a ningún sitio.";
     7: ! [2026-09-16] Faltaba -- se usa cuando quien se mueve es un PNJ
        ! (actor ~= player), no el jugador.
        print_ret (The) actor, " se marcha.";
    }
  Insert:
    ...
    !    de otro
    ! [2026-09-16 backlog #6, tanda 2] casos 1/4/5/9 nombran al actor si
    ! no es el jugador, igual que CSubjectVerb/CSubjectWill/CSubjectCant
    ! (actor,...) en inform6lib/english.h:994/1000/1003/1009. Casos
    ! 2/3/6/7/8 nombran a x1 o son impersonales (igual en inglés) -- se
    ! dejan como estaban.
    switch(n)
    {
     1: if (actor == player) {
            if (x1 has animate) "Antes tendrí", (a_) " que ", (coge)"rl", (o) x1,
                 " y no sé si se dejará",(n)x1,".";
            else
                 "Necesit", (as_) " tener ", (the) x1,
                 " para poder meter", (lo) x1,
                 " donde sea.";
        } else {
            if (x1 has animate) "", (The) actor, " antes tendrí", (a_) " que ",
                 (coge)"rl", (o) x1, " y no sé si se dejará",(n)x1,".";
            else
                 "", (The) actor, " necesit", (as_) " tener ", (the) x1,
                 " para poder meter", (lo) x1, " donde sea.";
        }
     2: "No se pueden meter cosas dentro ", (del) x1, ".";
     3: print_ret (The) x1, " ", (esta) x1, " cerrad", (o) x1, ".";
     4: if (actor == player) "", (_Tienes_) " que quitár", (se_) "", (lo) x1, " antes.";
        else "", (The) actor, " ", (tienes_) " que quitár", (se_) "", (lo) x1, " antes.";
     5: if (actor == player) "No ", (puedes_) " poner un objeto dentro de sí mismo.";
        else "", (The) actor, " no ", (puedes_) " poner un objeto dentro de sí mismo.";
     6: "(primero ", (se_) " ", (lo) x1, " quit", (as_) ")^";
     7: "No queda sitio en ", (the) x1, ".";
     8: "Hecho.";
     9: if (actor == player) "Met", (es_) " ", (the) x1, " dentro ", (del) second, ".";
        else "", (The) actor, " met", (es_) " ", (the) x1, " dentro ", (del) second, ".";
    }
 ...
 Inv:
    ...
    !  3 y 4: [6/11]
    ! [2026-09-16 backlog #6, tanda 2] casos 1/2 nombran al actor si no
    ! es el jugador, igual que CSubjectIs(actor,...) en
    ! inform6lib/english.h:1012-1013. Casos 3/4 son solo puntuación de
    ! cierre de la lista, sin sujeto -- sin cambios.
    switch(n)
    {
     1: if (actor == player) "No llev", (as_) " nada.";
        else "", (The) actor, " no llev", (as_) " nada.";
     2: if (actor == player) print "Llev", (as_) "";
        else print (The) actor, " llev", (as_) "";
     3:  print ":^";
     4:  print ".^";
    }
  ! [2026-09-16 backlog #6, tanda 2] nombra al actor si no es el
  ! jugador, igual que CSubjectVerb(actor,...) en
  ! inform6lib/english.h:1017.
  Jump: if (actor == player) "Salt", (as_) " en el sitio, sin ningún resultado.";
        else "", (The) actor, " salt", (as_) " en el sitio, sin ningún resultado.";
  ! [2026-09-16 backlog #6] JumpIn/JumpOn/JumpOver/Tie separados (antes
  ! JumpOver,Tie compartían un solo texto fijo sin distinguir n=1/n=2).
  ! En inglés, n=1 de JumpIn/JumpOn/JumpOver es un mensaje IMPERSONAL en
  ! gerundio ("Jumping in X would achieve nothing here" -- sin
  ! CSubject*, nadie se nombra), pero el n=1 de Tie SÍ usa
  ! CSubjectVerb(actor,...) ("You would achieve nothing by this.") --
  ! por eso no podían resolverse con el mismo patrón mecánico y hacía
  ! falta separarlos primero. El n=2 de las 4 es DecideAgainst() en
  ! inglés (mismo mensaje genérico en las 4) -> DecidirEnContra() acá.
  JumpIn: switch(n) {
     1: "Saltar dentro de ", (the) x1, " no lograría nada aquí.";
     2: DecidirEnContra();
    }
  JumpOn: switch(n) {
     1: "Saltar encima de ", (the) x1, " no lograría nada aquí.";
     2: DecidirEnContra();
    }
  JumpOver: switch(n) {
     1: "Saltar por encima de ", (the) x1, " no lograría nada aquí.";
     2: DecidirEnContra();
    }
  Tie: switch(n) {
     1: if (actor == player) "No lograr", (as__) " nada así.";
        else "", (The) actor, " no lograr", (as__) " nada así.";
     2: DecidirEnContra();
    }
  Kiss:         "No creo que deb", (a_) ".";
  ! [2026-09-16 backlog #6, tanda 2] nombra al actor si no es el
  ! jugador, igual que CSubjectVerb(actor,...) en
  ! inform6lib/english.h:1037.
  Listen: if (actor == player) "No escuch", (as_) " nada fuera de lo común.";
          else "", (The) actor, " no escuch", (as_) " nada fuera de lo común.";
  ListMiscellany:
...
! ************************************
 ...
 LMode3:
   ...
   Lock:
    ...
    switch(n)
    {
      ! [2026-09-16 backlog #6, tanda 3] casos 3/5 nombran al actor si
      ! no es el jugador, igual que CSubjectWill/CSubjectVerb(actor,...)
      ! en inform6lib/english.h:1080/1082. Casos 1/2/4 nombran a x1 (el
      ! objeto), no al actor -- sin cambios.
      ...
      3: if (actor == player) "Primero tendr", (as__) " que cerrar ", (the) x1, ".";
         else "", (The) actor, " primero tendr", (as__) " que cerrar ", (the) x1, ".";
      4: if (second) "No parece", (n) x1, " encajar en la cerradura.";
    "Necesit", (as_) " algún tipo de llave.";
      5: if (actor == player) {
             if (second) "", (_Cierras_) " ", (the) x1," con ", (the) second, ".";
             "Ech", (as_) " el cerrojo ", (al) x1, ".";
         } else {
             if (second) "", (The) actor, " ", (cierras_) " ", (the) x1,
                 " con ", (the) second, ".";
             "", (The) actor, " ech", (as_) " el cerrojo ", (al) x1, ".";
         }
    }
 ...
 Look:
    ...
    ".";
     ! [2026-09-16 backlog #6, tanda 3] rama x1==location (mirar la
     ! localidad en sí, el caso normal de "mirar") nombra al actor si no
     ! es el jugador, igual que CSubjectCan(actor,...) en
     ! inform6lib/english.h:1098. Rama x1~=location (mirar sobre/dentro
     ! de un soporte/recipiente) menciona a actor de rebote, como el
     ! inglés (theActor) -- sin cambios. Caso 7 nombra al actor igual
     ! que CSubjectVerb(actor,...) en inform6lib/english.h:1104.
     5,6: new_line; ! ELIUK: agregado salto de linea inicial (solo I6)
         ...
         else if (actor == player) print "", (_Puedes_) " ver ";
         else print (The) actor, " ", (puedes_) " ver ";
         ...
         if (x1~=location) ".";
         ".";
     7: if (actor == player) "No observ", (as_) " nada digno de mención al mirar hacia ", (the) x1, ".";
        else "", (The) actor, " no observ", (as_) " nada digno de mención al mirar hacia ", (the) x1, ".";
  ...
  LookUnder:
    ...
    {
     1: "Está muy oscuro.";
     ! [2026-09-16 backlog #6, tanda 3] nombra al actor si no es el
     ! jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1110.
     2: if (actor == player) "No v", (eo_) " nada interesante.";
        else "", (The) actor, " no v", (eo_) " nada interesante.";
    }
...
!          TextoLlamativo(" Has ganado ");
    ...
    ! mensaje [Miscellany,5]
     ...
     9: "^¡", (_Refl_) " h", (e_) " quedado a oscuras!"; ! ELIUK: agregado salto de linea inicial (solo I6)
...
!#ENDIF; ! NO_PUNTUACION
    ...
    ! tecla
     ...
     58: ! [inform6lib, sin portar hasta 2026-09-16] Igual que el 26
    ! (avisa que el parser va a coger el objeto antes de actuar),
    ! pero cuando ese objeto está dentro de un contenedor o encima
    ! de un soporte: agrega de dónde lo va a sacar. Disparado desde
    ! ImplicitTake/ImplicitInsert en verblib.h (heredado, no
    ! reemplazado por INFSPR.h), así que sí se ejecuta en un juego
    ! en español aunque nunca haya estado traducido.
    print "(primero trat", (as_) " de ", (coge) "r ";
        if (x1 has animate) print (al) x1; else print (the) x1;
        if (x2 has supporter) print " de encima de "; else print " de dentro de ";
        print (the) x2;
    ")";

     59: ! [inform6lib, sin portar hasta 2026-09-16] El player pidió
    ! "coge todo"/"deja todo" etc. con más objetos de los que
    ! take_all_rule permite procesar de una — se le pide que sea
    ! más preciso. Disparado desde parser.h (heredado).
    "", (_Tienes_) " que ser más específic", (o) player, ".";
    }
  ...
  ! [2026-09-16] NotifyOn faltaba por completo -- alcanzable con
  ! "notificar si"/"notify on" (SpanishG.h), pero sin mensaje no decía
  ! nada. Encontrado en auditoría sistemática contra los call sites
  ! reales de inform6lib (ver sync-progreso.md).
  NotifyOn:
...
!     10: print "   (perdid", (o) x1, ")";
  ...
  Open:
    ...
    switch(n)
    {
     ...
     ! [2026-09-16 backlog #6, tanda 4] casos 4/5 nombran al actor si no
     ! es el jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1245/1249. Casos 1/2/3/6 nombran a x1 o son
     ! impersonales/de rebote (igual en inglés) -- sin cambios.
     4: if (actor == player) print "Abr", (_es_) " ", (the) x1, ", descubriendo ";
        else print (The) actor, " abr", (_es_) " ", (the) x1, ", descubriendo ";
    if (WriteListFrom(child(x1),
                            ENGLISH_BIT + TERSE_BIT + CONCEAL_BIT)==0) "nada.";
    ".";
     5: if (actor == player) "Abr", (_es_) " ", (the) x1, ".";
        else "", (The) actor, " abr", (_es_) " ", (the) x1, ".";
     6: ! [2026-09-16] Faltaba -- lo llama ImplicitOpen (verblib.h,
        ! heredado), mismo patrón que Close caso 4.
        "(primero abr", (_es_) " ", (the) x1, ")";
    }
  ...
  Places: 
    ...
    switch (n) {
        ...
        2:  print ".^";
        3:  ! [2026-09-16] Faltaba -- se usa cuando no se ha visitado
            ! ningún sitio todavía, en vez de 1+lista+2.
            print "No has visitado nada.";
    }

  Pray: print "No ob", (tienes_) " nada práctico de ";
        ! [2026-09-16] Mismo fix actor-aware que los 35 helpers de
        ! conjugación (ver la nota junto a ActorNarrativeVoice/
        ! ActorNarrativeNumber) -- este bloque no usa tu_/_Tu_ porque
        ! Ricpelo lo escribió a mano para fijar el género en femenino
        ! ("oraciones"), pero tenía el mismo bug: miraba
        ! player.narrative_voice directo sin importar quién reza.
        if (ActorNarrativeVoice()) {
            if (ActorNarrativeNumber() == PLURAL_NUMBER) {
                switch (ActorNarrativeVoice()) {
                  1: print "nuestra";
                  3: print "su";
                  default: print "vuestra"; ! Segunda persona
                }
  ...
  Pull, Push, Turn:
  ! [2026-09-16] Renumerado -- la numeración vieja (1=estático,
  ! 2=escenario, 3=genérico, 4=animado) estaba corrida respecto de los
  ! call sites reales de PullSub/PushSub/TurnSub (verblib.h, heredados):
  ! el contenido de cada caso era correcto, pero vivía un número más
  ! abajo de lo que el código realmente pide (mismo tipo de bug que
  ! Drop). Esquema real:
  !   1: el objeto ES el jugador (tirar/empujar/girarse a sí mismo)
  !   2: el objeto tiene 'estatico'
  !   3: el objeto tiene 'escenario'
  !   4: caso genérico (ni estático ni escenario ni animado)
  !   5: el objeto es animado, o (para Push/Turn) es el actor
  !   6: el objeto es el actor, solo para Pull (Push/Turn usan 5 para
  !      esto también)
    switch(n)
    {
      ! [2026-09-16 backlog #6, tanda 4] caso 3 nombra al actor si no es
      ! el jugador, igual que CSubjectIs(actor,...) en
      ! inform6lib/english.h:1279. Caso 6 reusa DecidirEnContra(), igual
      ! que CSubjectVerb(actor,...) dentro de DecideAgainst()
      ! (english.h:1286) -- mismo mecanismo que el n=2 de
      ! Jump{In,On,Over}/Tie. Caso 1 es un caso especial de x1==player
      ! que el propio inglés resuelve mirando "player" directo, no
      ! "actor" -- sin CSubject*, sin cambios. Casos 2/4/5 nombran a x1
      ! o son impersonales (igual en inglés) -- sin cambios.
      1: "Castigar", (se_) " así no sirve de nada.";
      2: "Está", (n)x1, " firmemente sujet", (o)x1, ".";
      3: if (actor == player) "No ", (eres_) " capa", (z_) ".";
         else "", (The) actor, " no ", (eres_) " capa", (z_) ".";
      4: "No ocurre nada, aparentemente.";
      5: "Eso sería, como poco, maleducado.";
      6: DecidirEnContra();
    }
...
! Push: see Pull    
 ...
 PutOn:
    ...
    !    otro.
    ! [2026-09-16 backlog #6, tanda 5] casos 1/2/4/8 nombran al actor si
    ! no es el jugador, igual que CSubjectVerb/CSubjectCant(actor,...)
    ! en inform6lib/english.h:1301/1305/1309/1315. Casos 3/5/6/7 son
    ! impersonales (igual en inglés) -- sin cambios.
    switch(n)
    {
     1: if (actor == player) {
            if (x1 has animate)
                ...
                "Necesit", (as_) " tener ", (the) x1,
                    " para poder poner", (lo) x1, " donde sea.";
        } else {
            if (x1 has animate)
                "", (The) actor, " antes tendr", (ias_) " que", (coge) "rl",
                    (o) x1, ", y no ", (_se_) " si se dejará", (n) x1, ".";
            else
                "", (The) actor, " necesit", (as_) " tener ", (the) x1,
                    " para poder poner", (lo) x1, " donde sea.";
        }
     2: if (actor == player) "No ", (puedes_) " poner un objeto sobre sí mismo.";
        else "", (The) actor, " no ", (puedes_) " poner un objeto sobre sí mismo.";
     3: "Poner cosas sobre ", (the) x1, " no servirá de nada.";
     4: if (actor == player) "", (_Te_) " falta destreza.";
        else "", (The) actor, " ", (te_) " falta destreza.";
     5: "(primero ", (se_) " ", (lo) x1, " quit", (as_) ")^";
     6: "No queda sitio en ", (the) x1, " para poner nada más.";
     7: "Hecho.";
     8: if (actor == player) "Coloc", (as_) " ", (the) x1, " sobre ", (the) second, ".";
        else "", (The) actor, " coloc", (as_) " ", (the) x1, " sobre ", (the) second, ".";
    }
 ...
 Remove:
    ...
    "Sacad", (o) x1, ".";
     4: ! [2026-09-16] Faltaba -- se usa cuando el objeto no está ni
        ! dentro ni encima de nada (parent no es container ni supporter).
        "¡Pero si no ", (esta) x1, " dentro ni encima de nada!";
    }
 ...
 ! [2026-09-16 backlog #6] separado en n=1/n=2 -- el texto fijo único
 ! que traía la librería ("Ya está bastante limpio.", sujeto x1) no
 ! correspondía a NINGUNO de los 2 casos reales de RubSub (verblib.h:
 ! n=2 si el objeto es animado -> DecideAgainst() en inglés; n=1 caso
 ! genérico -> CSubjectVerb(actor,"achieve",...), "you achieve nothing
 ! by this"). Se reemplaza por contenido fiel a esos 2 casos, mismo
 ! idiom ya usado para Tie/JumpIn/JumpOn/JumpOver ("lograr" + as__).
 Rub: switch(n) {
    1: if (actor == player) "No lograr", (as__) " nada frotando eso.";
       else "", (The) actor, " no lograr", (as__) " nada frotando eso.";
    2: DecidirEnContra();
   }
 ...
 Save:
    ...
    ! [6/11] agregado de (2)
  Score: switch (n) {
    ! [2026-09-17 backlog #2] "puntuación" es femenino -- usa
    ! TuFemenino_, no tu_ (que da "nuestro"/"vuestro" en plural). Antes
    ! de este fix, tu_ derivaba el género del JUGADOR en vez del
    ! sustantivo, dando "nuestro" para un jugador varón aunque
    ! "puntuación" siempre es femenino.
    1: if (deadflag) print "En este juego, ", (TuFemenino_) " puntuación ha sido ";
       else print "Hasta el momento ", (TuFemenino_) " puntuación es ";
 ...
 ScriptOn:
  ...
  Search:
    ...
    ".";
     ! [2026-09-16 backlog #6, tanda 5] casos 4/5 nombran al actor si no
     ! es el jugador, igual que CSubjectVerb/CSubjectCant(actor,...) en
     ! inform6lib/english.h:1403/1404.
     4: if (actor == player) "No ", (encuentro_) " nada interesante.";
        else "", (The) actor, " no ", (encuentro_) " nada interesante.";
     5: if (actor == player) "No ", (puedes_) " ver lo que hay dentro ", (del) x1,
            " porque ", (esta) x1, " cerrad", (o)x1, ".";
        else "", (The) actor, " no ", (puedes_) " ver lo que hay dentro ", (del) x1,
            " porque ", (esta) x1, " cerrad", (o)x1, ".";
  ...
  ! [2026-09-16 backlog #6, tanda 5] nombra al actor si no es el
  ! jugador, igual que CSubjectCant(actor,...) en
  ! inform6lib/english.h:1412.
  Set: if (actor == player) "No, no ", (puedes_) ".";
       else "No, ", (the) actor, " no ", (puedes_) ".";
  ...
  Show:
    ...
    switch(n)
    {
     ! [2026-09-16 backlog #6, tanda 5] caso 1 nombra al actor si no es
     ! el jugador, igual que CSubjectIsnt(actor,...) en
     ! inform6lib/english.h:1415. Caso 2 nombra a x1 (el PNJ al que se
     ! le muestra algo), no al actor -- ya lo hacía bien.
     1: if (actor == player) "No ", (tienes_) " ", (the)x1, ".";
        else "", (The) actor, " no ", (tienes_) " ", (the)x1, ".";
  ...
  ! [2026-09-16 backlog #6, tanda 5] nombra al actor si no es el
  ! jugador, igual que CSubjectIsnt(actor,...) en
  ! inform6lib/english.h:1421. De paso, bug real encontrado: la
  ! concordancia de género de "somnoliento/a" miraba "player" fijo en
  ! vez de "actor" -- un PNJ hembra hubiera dado la forma masculina.
  Sleep: if (actor == player) "No est", (oy_) " especialmente somnolient", (o) player, ".";
         else "", (The) actor, " no est", (oy_) " especialmente somnolient", (o) actor, ".";
  ! [2026-09-16 backlog #6, tanda 5] separado en n=1/n=2 -- antes un
  ! solo texto fijo (el contenido de n=1) que nunca cubría el n=2 real
  ! (objetivo animado -> DecideAgainst() en inglés, mismo patrón que se
  ! resolvió para Jump{In,On,Over}/Tie). n=1 nombra al actor igual que
  ! CSubjectVerb(actor,...) en inform6lib/english.h:1423.
  Smell: switch(n) {
     1: if (actor == player) "No ", (hueles_) " nada extraño.";
        else "", (The) actor, " no ", (hueles_) " nada extraño.";
     2: DecidirEnContra();
    }
 ...
 Squeeze:
    ...
    switch(n)
    {
     ! [2026-09-16 backlog #6, tanda 5] caso 2 nombra al actor si no es
     ! el jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1433.
     ! [2026-09-17 backlog #7] caso 1 reusa DecidirEnContra(), igual que
     ! el inglés (DecideAgainst() en english.h:1432) -- reemplaza el
     ! texto propio "¡Las manos quietas!" que no correspondía al caso.
     1: DecidirEnContra();
     2: if (actor == player) "No consig", (ues_) " nada haciendo eso.";
        else "", (The) actor, " no consig", (ues_) " nada haciendo eso.";
    }
  ...
  SwitchOff:
    ...
    switch(n)
    {
     ! [2026-09-16 backlog #6, tanda 6] caso 3 nombra al actor si no es
     ! el jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1450.
     1: print_ret "No es algo que pueda apagarse.";
     2: print_ret "Ya estaba", (n) x1, " apagad", (o) x1, ".";
     3: if (actor == player) "Apag", (as_) " ", (the) x1, ".";
        else "", (The) actor, " apag", (as_) " ", (the) x1, ".";
    }
  ...
  SwitchOn:
    ...
    switch(n)
    {
     ! [2026-09-16 backlog #6, tanda 6] caso 3 nombra al actor si no es
     ! el jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1458.
     1: print_ret "No es algo que pueda encenderse.";
     2: print_ret "Ya estaba", (n) x1, " encendid", (o) x1, ".";
     3: if (actor == player) "Enciend", (es_) " ", (the) x1, ".";
        else "", (The) actor, " enciend", (es_) " ", (the) x1, ".";
    }
...
!     1: "Hecho."; ! [infsp] fixed, no se pasa el objeto
     ! [2026-09-16 backlog #6, tanda 6] casos 2/4/5/12 nombran al actor
     ! si no es el jugador, igual que CSubjectIs/CSubjectWill/
     ! CSubjectVerb(actor,...) en inform6lib/english.h:1463/1467/1471/
     ! 1478.
     ! [2026-09-17 backlog #8] caso 10 también nombra al actor -- el
     ! inglés nombra a x1 ahí (no al actor), pero el usuario decidió
     ! ampliar el criterio: el español ya tenía sujeto-actor propio
     ! elidido ("podrías"), y la conjugación con (ias_)/(se_) ya era
     ! actor-aware, solo faltaba decir quién.
     1: if (dialecto_sudamericano) print "Tomad";
       else print "Cogid";
       print_ret (o) noun, ".";
     2: if (actor == player) "Siempre ", (se_) " ", (tienes_) " a ", (ti_) " mism", (o) player, ".";
        else "", (The) actor, " siempre ", (se_) " ", (tienes_) " a ", (ti_) " mism", (o) actor, ".";
     3: "No creo que ", (al) x1, " le", (s) x1, " gustara.";
     4: if (actor == player) print "", (_Tienes_) " que ";
        else print (The) actor, " ", (tienes_) " que ";
    if (x1 has supporter) print "bajar", (se_) " "; else print "salir", (se_) " ";
    print_ret (del) x1, " antes.";
     5: if (actor == player) "Ya ", (tienes_) " ", (the) x1, ".";
        else "", (The) actor, " ya ", (tienes_) " ", (the) x1, ".";
     ...
     10: if (actor == player) "Difícilmente podr", (ias_) " llevar", (se_) " eso.";
         else "", (The) actor, " difícilmente podr", (ias_) " llevar", (se_) " eso.";
     11: print_ret (The) x1, " está fij", (o) x1, " en el sitio.";
     12: if (actor == player) "Ya llev", (as_) " demasiadas cosas.";
         else "", (The) actor, " ya llev", (as_) " demasiadas cosas.";
     13: "(coloc", (as_) " ", (the) x1, " en ", (the) SACK_OBJECT,
         " para hacer sitio)";
    }

   ! [2026-09-16 backlog #6, tanda 6] separado en n=1/n=2 -- mismo
   ! patrón que Smell/Taste (verblib.h: TasteSub, n=2 si el objeto es
   ! animado -> DecideAgainst() en inglés). n=1 nombra al actor igual
   ! que CSubjectVerb(actor,...) en inform6lib/english.h:1482; el texto
   ! ya existente correspondía correctamente a n=1, solo faltaba n=2.
   Taste: switch(n) {
      1: if (actor == player) "No sabore", (as_) " nada inesperado.";
         else "", (The) actor, " no sabore", (as_) " nada inesperado.";
      2: DecidirEnContra();
     }
   ...
   Tell:
    ...
    switch(n)
    {
     ! [2026-09-16 backlog #6, tanda 6] caso 1 nombra al actor si no es
     ! el jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1486. Tenía además el mismo bug de género
     ! que Sleep/Take#2 ((o) player en vez de (o) actor).
     ! [2026-09-17 backlog #8] caso 2 también nombra al actor -- el
     ! inglés es impersonal ahí, pero el usuario decidió ampliar el
     ! criterio: el español ya tenía sujeto-actor propio elidido ("he
     ! provocado"), y la conjugación con (e_) ya era actor-aware, solo
     ! faltaba decir quién.
     1: if (actor == player) "Habl", (as_) " sol", (o) player, " durante un rato.";
        else "", (The) actor, " habl", (as_) " sol", (o) actor, " durante un rato.";
     2: if (actor == player) "No h", (e_) " provocado ninguna reacción.";
        else "", (The) actor, " no h", (e_) " provocado ninguna reacción.";
    }
   ...
   ThrowAt:
    ...
    switch(n)
    {
     ! [2026-09-16 backlog #6, tanda 6] caso 2 nombra al actor si no es
     ! el jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1495.
     1: "No serviría de nada.";
     2: if (actor == player) "En el último momento ", (se_) " ech", (as_) " atrás.";
        else "En el último momento ", (the) actor, " ", (se_) " ech", (as_) " atrás.";
    }
  ! [2026-09-16] Transfer no tenía NINGÚN mensaje -- lo llama TransferSub
  ! (verblib.h, heredado) cuando "transferir X a Y" termina con X de
  ! vuelta en poder del jugador (p.ej. intentar transferírselo a sí
  ! mismo). Encontrado en auditoría sistemática contra los call sites
  ! reales de inform6lib (ver sync-progreso.md).
  ! [2026-09-16 backlog #6, tanda 6] nombra al actor si no es el
  ! jugador, igual que CSubjectIs(actor,...) en
  ! inform6lib/english.h:1514.
  Transfer: if (actor == player) "Ya ", (lo) x1, " ", (tienes_) ".";
            else "", (The) actor, " ya ", (lo) x1, " ", (tienes_) ".";
! Tie: separado de JumpOver, ver más arriba (backlog #6, 2026-09-16).
  Touch:
    ...
    ! [2026-09-16 backlog #6, tanda 6] caso 2 nombra al actor si no es
    ! el jugador, igual que CSubjectVerb(actor,...) en
    ! inform6lib/english.h:1508.
    ! [2026-09-17 backlog #7] caso 1 reusa DecidirEnContra(), igual que
    ! el inglés (DecideAgainst() en english.h:1507) -- reemplaza el
    ! texto propio "¡Las manos quietas!" que no correspondía al caso.
    ! [2026-09-17] bug real encontrado al investigar #8: caso 3 tenía
    ! "crees" fijo (2ª persona hardcodeada), a diferencia de Take#10/
    ! Tell#2 que ya usaban helpers actor-aware ((ias_)/(e_)). Corregido
    ! a "cre"+(es_) (mismo idiom que el resto de verbos -er): da
    ! "creo"/"crees"/"cree" según ActorNarrativeVoice().
    ! [2026-09-17 backlog #8] caso 3 también nombra al actor -- el
    ! inglés es impersonal ahí, pero el usuario decidió ampliar el
    ! criterio: ya con el bug de conjugación arreglado, solo faltaba
    ! decir quién.
    switch(n)
    {
      1: DecidirEnContra();
      2: if (actor == player) "No not", (as_) " nada extraño al tacto.";
         else "", (The) actor, " no not", (as_) " nada extraño al tacto.";
      3: if (actor == player) "Si cre", (es_) " que eso servirá de algo...";
         else "Si ", (the) actor, " cre", (es_) " que eso servirá de algo...";
    }
! Turn: see Pull.
  ...
  Unlock:
    ...
    switch(n)
    {
     ! [2026-09-16 backlog #6, tanda 6] caso 4 nombra al actor si no es
     ! el jugador, igual que CSubjectVerb(actor,...) en
     ! inform6lib/english.h:1525. Casos 1/2/3 nombran a x1 (mismo
     ! criterio que Lock en tanda 3) -- sin cambios.
     1: "No parece", (n) x1," tener ningún tipo de cerrojo.";
     2:    print_ret (The) x1, " ya tenía", (n) x1, " abierto el cerrojo.";
     3: if (second) "No parece", (n) x1, " encajar en la cerradura.";
    "Necesit", (as_) " algún tipo de llave.";
     4: if (actor == player) {
            if (second) "Quit", (as_) " el cerrojo ", (al) x1, " con ", (the) second, ".";
            "Quit", (as_) " el cerrojo ", (al) x1, ".";
        } else {
            if (second) "", (The) actor, " quit", (as_) " el cerrojo ", (al) x1,
                " con ", (the) second, ".";
            "", (The) actor, " quit", (as_) " el cerrojo ", (al) x1, ".";
        }
    }

 ! [2026-09-16 backlog #6, tanda 6] nombra al actor si no es el
 ! jugador, igual que CSubjectWill(actor) en inform6lib/english.h:1528.
 VagueGo: ! El jugador ha dicho IR sin especificar a dónde
    if (actor == player) "", (_Tienes_) " que especificar en qué dirección ir.";
    else "", (The) actor, " ", (tienes_) " que especificar en qué dirección ir.";
 ...
 Verify:
  ...
  !  2: Si se agita un objeto que sí está en el inventario
    ! [2026-09-16 backlog #6, tanda 6] casos 1/2 nombran al actor si no
    ! es el jugador, igual que CSubjectIsnt/CSubjectVerb(actor,...) en
    ! inform6lib/english.h:1547/1548. Caso 2 tenía además el mismo bug
    ! de género que Sleep/Take#2/Tell#1 ((o) player en vez de (o)
    ! actor). Caso 3 queda pendiente (el inglés usa DecideAgainst() ahí
    ! pero el español tiene su propio texto), ver hallazgo en
    ! sync-progreso.md.
    switch (n)
    {
      1: if (actor == player) "No l",(o) x1, " ", (tienes_) ".";
         else "", (The) actor, " no l", (o) x1, " ", (tienes_) ".";
      2: if (actor == player) "", (_Refl_) " ", (sientes_) " ridícul", (o) player,
             " agitando ", (the) x1, ".";
         else "", (The) actor, " ", (se_) " ", (sientes_) " ridícul", (o) actor,
             " agitando ", (the) x1, ".";
      ! [2026-09-17 backlog #7] caso 3 reusa DecidirEnContra(), igual
      ! que el inglés (DecideAgainst() en english.h:1553) -- reemplaza
      ! el texto propio "No creo que deba." que no correspondía al
      ! caso. Se usa cuando se intenta agitar al propio actor (noun ==
      ! actor).
      3: DecidirEnContra();
    }
   ! [2026-09-16 backlog #6, tanda 6] nombra al actor si no es el
   ! jugador, igual que CSubjectVerb(actor,...) en
   ! inform6lib/english.h:1556. Mismo bug de género que Sleep/etc.
   ! ((o) player en vez de (o) actor).
   WaveHands:
    if (actor == player) "", (_Refl_) " v", (eo_) " ridícul", (o) player, " gesticulando así.";
    else "", (The) actor, " ", (se_) " v", (eo_) " ridícul", (o) actor, " gesticulando así.";
  ...
  ! mano", "sacude la mano", "saluda con la mano"...
   Wear:
    ...
    !  4: Exito.
    ! [2026-09-16 backlog #6, tanda 6] casos 1-5 nombran al actor si no
    ! es el jugador, igual que CSubjectCant/CSubjectIs/
    ! CSubjectVerb(actor,...) en inform6lib/english.h:1563-1567.
    switch(n)
    {
     1: if (actor == player) "¡No ", (puedes_) " poner", (se_) " eso!";
        else "¡", (The) actor, " no ", (puedes_) " poner", (se_) " eso!";
     2: if (actor == player) "No ", (lo) x1, " ", (tienes_) ".";
        else "", (The) actor, " no ", (lo) x1, " ", (tienes_) ".";
     3: if (actor == player) "¡Ya ", (lo) x1, " llev", (as_) " puest", (o) x1, "!.";
        else "¡Ya ", (the) actor, " ", (lo) x1, " llev", (as_) " puest", (o) x1, "!.";
     4: if (actor == player) "", (_Refl_) " ", (pones_) " ", (the) x1, ".";
        else "", (The) actor, " ", (se_) " ", (pones_) " ", (the) x1, ".";
     5: ! [2026-09-16] Faltaba -- no_implicit_actions activo (caso raro):
        ! hay que sacar x1 de x2 a mano antes de poder ponérselo.
        if (actor == player) print (_Tienes_) " que coger primero ";
        else print (The) actor, " ", (tienes_) " que coger primero ";
        if (x1 has animate) print (al) x1; else print (the) x1;
        if (x2 has supporter) print " de encima de "; else print " de dentro de ";
        print (the) x2, " antes de poder poner", (se_) " ", (the) x1, ".";
    }
! Yes:  see No.
];
#endif; ! SIN_MENSAJES
